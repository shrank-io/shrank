...
The Rust backend calls vllm-mlx directly via its OpenAI-compatible API (`/v1/chat/completions`, `/v1/embeddings`). Prompt building, JSON parsing, and relationship inference all happen in Rust. The `inference/` directory contains the vllm-mlx launcher script.
...
- `docs/PLAN-INFERENCE.md` — inference (vllm-mlx) setup plan
